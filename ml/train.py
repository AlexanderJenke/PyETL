...
if __name__ == '__main__':
    ...
    for epoch in range(N_EPOCHS):
        ...
        for batch in tqdm(trainloader, desc=f"Train ep{str(epoch).zfill(3)}"):
            ...
            # loss = loss_f1(output, labels) + loss_wMSE(output, labels) / 1e-2  # 5.6e-3
            f1, p, r = loss_f1_score(output, labels)
            wmse = loss_wMSE(output, labels)

            loss = (torch.tensor(1) - p) + (torch.tensor(1) - r) / 100 + wmse

            # loss = (torch.tensor(1) - p) / 100 + wmse
        ...
        f1, p, r = f1_score(preds_cat, labels_cat)
